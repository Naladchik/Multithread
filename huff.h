...
uint32_t zip(uint8_t * bin_buff, char * text_buf, struct Node ** nodes_arr, uint32_t* lut_codes, uint8_t* lut_lengths, uint8_t nodes_len);  //compression function (needs text buffer and codes to be prepared)
void unzip(uint8_t * bin_buff, char * text_buf, struct Node * hf_root); //decompression function (needs bytes buffer and Huffman tree to be prepared)
...
uint8_t MakeFrequencies(char * source_file, char * text_buf, struct Node ** nodes_arr); // makes input array of nodes already sorted and ready for the Huffman tree building
